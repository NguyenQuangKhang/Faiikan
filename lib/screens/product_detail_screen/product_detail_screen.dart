...
class _ProductDetailState extends State<ProductDetail>
  ...
  void initState() {
    ...
    _homeTween = ColorTween(begin: Colors.white, end: Colors.blue)
        .animate(_ColorAnimationController);
    _workOutTween = ColorTween(begin: Colors.black.withOpacity(0.5), end: Colors.black.withOpacity(0.1))
        .animate(_ColorAnimationController);
    ...
    return Scaffold(
      body: BlocBuilder<ProductDetailBloc, ProductDetailState>(
        builder: (context, state) {
          if (state is LoadingProductDetail || state is InitialProductDetail )
                      ...
                      Expanded(
                        child: NotificationListener<ScrollNotification>(
                          onNotification: (scrollInfo) {
                            ...
                            if(scrollInfo.metrics.pixels== scrollInfo.metrics.minScrollExtent&&scrollInfo is ScrollEndNotification && start==true)
                              {
                                start=false;
                                print("reset");
                                productDetailBloc.add(ProductDetailResetEvent(id: productDetailBloc.productDetail.id, person_id: widget.userId.toString()));
                              }
                            if (scrollInfo.metrics.axis == Axis.vertical) {
                              _ColorAnimationController.animateTo(scrollInfo.metrics.pixels/(MediaQuery.of(context).size.height*0.35-80));
                              if(scrollInfo.metrics.pixels>=MediaQuery.of(context).size.height*0.35-100 && elevation ==0)
                                {
                                  elevation=1;
                                }
                              if(scrollInfo.metrics.pixels<MediaQuery.of(context).size.height*0.35-100)
                                {
                                  elevation=0;
                                }
                              return scroll = true;
                              ...
                              child: InkWell(
                                onTap: () {
                                  ...
                                  if (widget.isNavigatedFromFavorite) {
                                    context
                                        .read<FavoriteBloc>()
                                        .add(FavoriteTap(person_id: widget.userId.toString(),index: context
                                        .read<FavoriteBloc>().index,product_id: widget.productId.toString()));
                                  }
                        ...
                        builder: (context, child) {
                          return Positioned(
                            ...
                            top: (MediaQuery.of(context).size.height*0.35 +
                                    180) -
                                (MediaQuery.of(context).size.height*0.35 + 150) *
                                    animation.value,
...
//                        width: 55 - animation.value * 30,
                  ...
                  if(state is LoadingProductDetailReset)
